def calc_g(start):
    """Calculate distance from start to its neighbors and return as a dictionary."""
    distances = {}
    for i in road[start]:
        dist = calc_dist(intersection.get(start), intersection.get(i))
        distances[i]=dist
    return distances

def calc_f(g):
    """Return min of segment distance from start(g) + estimated distance to goa(h)). """
    f_values = {}
    for k,v in g.items():
        f = v + estimated_distances_to_goal[k]
        f_values[k] = f

    min_value = min(f_values.values())
    for k, v in f_values.items():
         if v == min_value:
             return k
def shortest_path(start, goal):
    """
    1. Create h. h is a list of estimated distances from each point to the goal.
    2. Use a recursive function to calculate the shortest path to the goal from the start.
    :param start: Start of path
    :param goal: End of path
    :return: List of points that make up the shortest path
    """
    calc_h(goal)
    recursive_result = recursive_shortest_path(start, goal)
    shortest_path = [start]
    shortest_path.extend(recursive_result)
    print("shortest path called")
    return shortest_path

def recursive_shortest_path(start, goal):
    """
    1. Get dictionary(g) of distances between start and its neighbors.
    2. Get the min of segment distance from start(g) + estimated distance to goal(h)) from the calc_f function.
    3. Recursively repeat 1 and 2 till the goal is reached. Return the shortest path list.
    """
    path_list = []
    if start == goal:
        return []
        ...
        g = calc_g(start)
        previous_node = calc_f(g)
        path_list.append(previous_node)
        return (path_list + recursive_shortest_path(previous_node, goal))
